...
impl Plugin {
    ...
    fn ac_headers_list(&mut self) -> List {
        ...
        list.append(&*format!("User-Agent: {}", "STALKER/1.0")).unwrap();
    ...
    fn set_access_token(
        &mut self, _: &Amx, token: AmxString
    ) -> AmxResult<bool> {
    ...
    fn set_session_status(
        &mut self, _: &Amx, name: AmxString, status: bool
    ) -> AmxResult<bool> {
        ...
        let sz_url = &*String::from(format!(
            "{}/perform.php?state={}&name={}", RAC_API_URL, state, name));
        ...
        if handle.response_code() != Ok(200) {
            log::error!(" >> ROCK-AC: <RAC_SetUserSessionStatus> HTTP response code {}",
    ...
    fn ban_user(
        &mut self, _: &Amx, name: AmxString, reason: AmxString
    ) -> AmxResult<bool> {
        ...
        let mut handle = Easy::new();
        handle.url(&*format!("{}/block.php?action=block&name={}&reason={}",
            RAC_API_URL, name, sz_reason.into_owned())).unwrap();
        handle.http_headers(self.ac_headers_list()).unwrap();
        handle.get(true).unwrap();
        ...
        if handle.response_code() != Ok(200) {
            log::error!(" >> ROCK-AC: <RAC_BanUser> HTTP response code {}",
                handle.response_code().unwrap());
            return Ok(false)
        }
    ...
    fn unban_user(
        ...
        let mut handle = Easy::new();
        handle.url(&*format!("{}/block.php?action=unblock&name={}",
            RAC_API_URL, name)).unwrap();
        handle.http_headers(self.ac_headers_list()).unwrap();
        handle.get(true).unwrap();
        ...
        if handle.response_code() != Ok(200) {
            log::error!(" >> ROCK-AC: <RAC_UnbanUser> HTTP response code {}",
                handle.response_code().unwrap());
            return Ok(false)
        }
